# secretary-bot

Telegram-бот «Секретарь»: принимает клиента, проводит короткое AI-интервью и
формирует структурированное техническое задание для администратора.

Полная спецификация: [`docs/TZ_telegram_bot_zayavki_v1.1.md`](docs/TZ_telegram_bot_zayavki_v1.1.md)
и [`docs/AI_Specification_v1.1.md`](docs/AI_Specification_v1.1.md).

## Стек

Python 3.12 · aiogram 3.x (webhook на aiohttp) · asyncpg + PostgreSQL (Supabase) ·
Groq (`gpt-oss-20b` интервью / `gpt-oss-120b` финальное ТЗ, Whisper STT) ·
OpenRouter fallback (`gemma-4-31b-it:free` интервью / `minimax-m3:free` финальное ТЗ) ·
Render.com (Web Service, free tier) · UptimeRobot.

## Архитектура

```
app/
  config.py          — настройки из env (pydantic-settings)
  constants.py        — состояния State Machine, статусы, словари
  main.py              — aiohttp-приложение, webhook-эндпоинт, health-check
  db/
    schema.sql          — DDL (идемпотентно, безопасно перезапускать)
    pool.py, repo.py     — asyncpg-пул и репозитории по сущностям
  ai/
    schemas.py            — pydantic-контракты ответов LLM
    prompts.py              — системные промпты и сборка user-промптов
    clients.py                — HTTP-клиенты Groq/OpenRouter/Whisper
    orchestrator.py            — retry → repair → fallback поверх LLM-вызовов
  services/
    application_flow.py — состояние заявки + вызовы AI Orchestrator
    tz_generator.py       — генерация финального ТЗ + проверка качества
    debounce.py, locks.py, rate_limiter.py — защита от гонок/спама
    telegraph.py, reminders.py, tz_quality.py
  bot/
    keyboards.py, texts.py, middlewares.py, router.py
    handlers/            — start/new/help/interview/confirmation/admin
```

## Локальный запуск

```bash
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
cp .env.example .env   # заполнить реальными значениями
python -m scripts.apply_schema   # применить схему БД (идемпотентно)
python -m app.main
```

Бот поднимает aiohttp-сервер на `PORT` (по умолчанию 8080) с вебхуком на
`WEBHOOK_PATH` и health-check на `/health`. Локально вебхук Telegram не
достучится без публичного URL — для локальной отладки проще всего
прогнать функции сервисного слоя напрямую (см. `scripts/apply_schema.py`
как пример) либо пробросить порт через ngrok/cloudflared и указать этот
адрес в `WEBHOOK_BASE_URL`.

## Деплой на Render (free tier)

Free tier Render не даёт Background Worker, поэтому бот работает как
**Web Service на вебхуках** (не long polling), а UptimeRobot пингует
`/health`, чтобы сервис не засыпал.

1. Создать Web Service на Render, подключить репозиторий (см. `render.yaml`).
2. Build command: `pip install -r requirements.txt`, start command:
   `python -m app.main`.
3. Проставить все переменные окружения из `.env.example` (см. ниже).
4. `WEBHOOK_BASE_URL` — публичный URL сервиса, который выдаст Render
   (`https://<service>.onrender.com`), без хвостового слэша.
5. При старте бот сам вызывает `setWebhook` и применяет схему БД — ничего
   вручную дополнительно накатывать не нужно (но `scripts/apply_schema.py`
   доступен для ручного прогона/миграций).
6. Настроить в UptimeRobot HTTP(s)-монитор на `https://<service>.onrender.com/health`.

### Два независимых "не засыпающих" механизма

Тут на самом деле спят по разным причинам две разные вещи, и решения не связаны:

- **Render засыпает от отсутствия HTTP-трафика.** Решается пунктом 6 выше —
  внешний пинг `/health` каждые несколько минут не даёт free-tier инстансу
  уйти в сон.
- **Supabase приостанавливает free-tier проект примерно после 7 дней без
  обращений к БД** — даже если сам бот исправно отвечает на вебхуки, но
  клиенты им не пользуются. Это лечится не снаружи (UptimeRobot не видит
  БД), а изнутри: `app/services/supabase_keepalive.py` — фоновый цикл,
  который раз в `SUPABASE_KEEPALIVE_INTERVAL_SECONDS` (по умолчанию 6 часов)
  делает `select 1` через тот же asyncpg-пул, что и всё остальное. Ничего
  дополнительно настраивать не нужно — таск стартует вместе с ботом
  (`on_startup` в `app/main.py`) и живёт, пока жив процесс.

  Тот же приём (независимый фоновый пинг Supabase) используется в других
  ботах на этом стеке — там, где подключение к Supabase идёт через
  `supabase-py`, пингуют через REST-клиент; здесь БД — обычный Postgres
  через `asyncpg`, поэтому проще и надёжнее пинговать напрямую SQL-запросом,
  без лишней зависимости.

## Переменные окружения

См. `.env.example` — полный список с комментариями. Ключевые:

| Переменная | Назначение |
|---|---|
| `BOT_TOKEN` | токен бота от @BotFather |
| `ADMIN_ID` | Telegram ID администратора (доступ к `/admin`) |
| `DATABASE_URL` | Postgres connection string (Supabase → Project Settings → Database) |
| `GROQ_API_KEY` | платный тариф Groq, доступ к `gpt-oss-20b`/`gpt-oss-120b`/Whisper |
| `OPENROUTER_API_KEY` | fallback-провайдер |
| `WEBHOOK_BASE_URL`, `WEBHOOK_SECRET` | публичный адрес сервиса и секрет для проверки заголовка Telegram |
| `TELEGRAPH_HELP_URL` | можно оставить пустым — страница `/help` создаётся автоматически при первом старте |

## Заметки по реализации относительно ТЗ v1.1

Часть пунктов ТЗ оставлена «на усмотрение реализации» (п. 65) — решения,
принятые в коде:

- **Срок запуска (п.16)** — состояние `WAITING_DEADLINE` вставлено между
  адаптивным интервью и экраном подтверждения: вопрос задаётся один раз,
  сразу как только AI-интервью решает, что информации достаточно
  (`action=understanding`), следующий ответ пользователя сохраняется как есть.
- **Напоминание о незавершённой сессии (п.55)** — фоновый цикл раз в 15
  минут проверяет заявки без активности дольше `INCOMPLETE_SESSION_REMINDER_HOURS`
  и без отправленного напоминания; текст/таймаут — на усмотрение, можно
  поменять в `app/services/reminders.py`.
- **Порог «хулиганства» (п.20.3)** — если тема уточняющего вопроса не
  меняется 3 раза подряд (`SAME_TOPIC_ABANDON_THRESHOLD`), заявка помечается
  `flagged_as_abuse` и закрывается с просьбой создать новую через `/new`.
- **Debounce/rate limit/lock** — реализованы в памяти процесса (не Redis):
  достаточно для одного инстанса Render free tier; при горизонтальном
  масштабировании потребуется вынести в Redis.

## v1.2 — правки после завершения заявки

Отдельный лёгкий AI-контур поверх уже завершённой (`status=completed`) заявки —
таблица `revisions` со своим независимым статусом (🆕/👀/🔧/✅), без цикла
«Добавить информацию», без quality-gate и без отдельного `.md`-файла (см.
`docs/TZ_telegram_bot_zayavki_v1.1.md` п.67-68 и `docs/AI_Specification_v1.1.md` §33).

Решения, принятые в коде там, где ТЗ оставляет implementation-детали открытыми:

- **Черновик правки — в памяти, не в БД.** Пока клиент не нажал «✅ Верно»,
  описание правки и «Как я понял» существуют только в
  `RevisionDraftRegistry` (`app/services/revision_drafts.py`). Строка в
  `revisions` появляется только при подтверждении — сам флоу задуман как
  однопроходный и не требует переживания рестарта на полпути (в отличие от
  основного интервью, где `project_context`/`state` персистентны).
- **Нет кнопки отмены на этапе «Как я понял».** ТЗ п.68.2 явно оставляет
  только «✅ Верно». Если клиент вместо этого присылает новое сообщение —
  оно трактуется как переформулировка правки, анализ прогоняется заново.
- **Точка входа для клиента — экран `/start`.** Кнопка «✏️ Предложить
  правку» показывается только для последней завершённой заявки клиента —
  выбора среди нескольких прошлых заявок в интерфейсе нет (ТЗ п.64), так что
  давать её для более старых завершённых заявок было бы противоречиво.
- **Автопереход 🆕→👀 у правок** — по аналогии с автопереходом статуса
  заявки при открытии карточки (п.43), хотя для правок это явно не
  прописано; выбрано для единообразия админ-интерфейса.

## Жёсткий лимит уточняющих вопросов (после жалоб на затянутый диалог)

ТЗ v1.1/AI Specification изначально описывали лимит в ~3 вопроса как
**мягкий ориентир**: после него модель должна "проверить, достаточно ли
информации", но при желании имела право продолжить интервью, если ей
казалось, что не хватает чего-то критичного — жёсткого потолка не было
(AI Specification §14/§32). На практике диалоги стабильно затягивались
дольше, чем ожидалось, и первое, что выяснилось при разборе — конфиг
`CLARIFYING_QUESTIONS_SOFT_LIMIT` вообще не был подключён к промпту:
порог "после скольки вопросов пора закругляться" был захардкожен как `3`
прямо в `build_interview_user_prompt`, в обход настройки.

По итогу лимит теперь `MAX_CLARIFYING_QUESTIONS` (тот же дефолт — 3,
переменная окружения переименована, но старое имя просто будет
проигнорировано — риска сломать текущий деплой на Render нет) и
применяется в два слоя:

1. **Промпт** — и системный, и генерируемый на каждый ход, теперь говорят
   не "желательно закругляться", а "лимит исчерпан — ты ОБЯЗАН перейти к
   `action=understanding`, `action=ask` запрещён", с явным счётчиком
   "уже спрошено N из максимум M".
2. **Код (`app/services/application_flow.py`)** — не полагается на то,
   что модель послушается с первого раза. Если после достижения лимита
   модель всё равно возвращает `action=ask`, ответ переопределяется на
   лету: клиенту показывается не вопрос, а собранное на данный момент
   понимание задачи (текст берётся из `project_context.summary`), заявка
   идёт дальше по обычному пути (вопрос про срок → подтверждение), а
   несобранные детали остаются как `⚠ УТОЧНИТЬ У КЛИЕНТА` в итоговом ТЗ —
   их администратор уточнит напрямую после подтверждения. Событие
   `clarifying_limit_enforced` пишется в `events`, чтобы видно было, как
   часто модели вообще требуется это принудительное вмешательство.

Проверено симуляцией модели, которая на каждом ходу настойчиво пытается
задать ещё один вопрос: ровно 3 вопроса доходят до клиента, дальше —
принудительный переход к пониманию, независимо от того, что "думает"
модель.

## Вопрос о сроке: найденный баг + quick-reply кнопки

При попытке сократить путь до заявки до "трёх кликов" обнаружился баг в
существующей логике: экран `WAITING_DEADLINE` показывал клиенту текст
"Как я понял вашу задачу" **без кнопок подтверждения** — сам вопрос
"Когда вам желательно запустить решение?" нигде не отправлялся, он был
только внутри системного промпта для парсинга ответа, а не в исходящем
сообщении. Клиент видел непонятный текст с одной кнопкой «Отменить» и
должен был понять по контексту, что нужно ответить именно про сроки.

Поправлено: `InterviewOutcome` для `ask_deadline` теперь несёт сам
вопрос (`DEADLINE_QUESTION_TEXT` в `application_flow.py`), а понимание
задачи отдельно хранится в `pending_understanding_message` и
показывается с кнопками ✅/➕/❌ на следующем шаге, как и было задумано.

Заодно — по просьбе сократить путь до заявки — вопрос о сроке теперь
идёт с тремя quick-reply кнопками (🚀 Как можно быстрее / 📅 В течение
месяца / 🤷 Сроков нет), которые эквивалентны соответствующему
свободному тексту и ведут по тому же коду, что и печатный ответ.
Свободный текст по-прежнему принимается — кнопки не замена, а быстрый
путь для типового ответа, чтобы не печатать его каждый раз.

## Режим User/Admin для тестирования от своего аккаунта

Админ может переключить своё же Telegram-подключение в режим **User** —
кнопка «🔀 Режим: User/Admin» в главном меню админки (`/admin`), либо
команда `/mode` в любой момент. В режиме User бот общается с админом как
с обычным потенциальным заказчиком: доступен полный путь оформления
заявки — `/start`, описание задачи, интервью, кнопки, финализация,
включая правки после завершения. Технически при этом создаётся реальная
запись в `users` по Telegram ID админа, и тестовые заявки — это реальные
строки в БД, видимые в самой админ-панели (админ получит на себя же
push-уведомление о «новой заявке» — это ожидаемо, так проверяется вся
цепочка целиком, а не только клиентская часть).

Важный нюанс реализации: `is_admin`, которым по всему коду проверяется
доступ, отражает **текущий выбранный режим**, а не сырой Telegram ID —
это позволило не трогать ни одного из существующих admin-хендлеров
(`admin.py`), они просто перестают совпадать с фильтром, пока включён
User. Но это создавало риск запереть админа в режиме User без выхода —
`/admin` в этом режиме корректно не отвечает, как и для настоящего
клиента. Поэтому `/mode` — единственная команда, которая проверяет сырой
`message.from_user.id`, а не вычисленный `is_admin`, и поэтому она
гарантированно работает в обоих режимах.

Режим хранится в памяти процесса (не в БД) и сбрасывается в Admin при
рестарте — специально: чтобы после передеплоя админ не остался молча в
режиме теста и не пропустил реальные уведомления о заявках.

## "Тишина" после ✅ Верно — три найденных проблемы

Репортилось: полный путь клиента до «Как я понял…», после нажатия
«✅ Верно» — тишина. В логах: `400 Bad Request` на `…/completions` и
`tz quality check failed, отсутствует раздел ИИ функциональность`.

1. **Настоящий баг, вызывавший тишину.** При ошибке финализации
   (`TZGenerationError`) бот отправлял `FINALIZATION_FAILED_MESSAGE`
   («нажмите ✅ Верно ещё раз») с клавиатурой `question_keyboard` — а в
   ней есть только «❌ Отменить», кнопки «✅ Верно» физически не было.
   Клиент получал инструкцию нажать кнопку, которой не существует, и
   застревал. Поправлено в `confirmation.py` — теперь используется
   `understanding_keyboard` с реальной кнопкой «Верно» (и «Добавить
   информацию», если лимит циклов не исчерпан).
2. **Хрупкая проверка разделов ТЗ, объясняющая конкретно эту ошибку.**
   `tz_quality.py` сравнивал заголовки разделов посимвольно, включая
   пунктуацию — «ИИ-функциональность» (с дефисом) не совпадало с тем,
   что реально написала модель, например «ИИ функциональность» (пробел)
   или любую другую безобидную вариацию пунктуации. Раздел по сути
   присутствовал, проверка просто была слишком буквальной. Теперь перед
   сравнением у заголовка и у документа убираются все символы, кроме
   букв/цифр — семантически то же самое продолжает ловиться как
   реальный пропуск, но форматирование ИИ больше не считается провалом.
3. **Слепая зона в логах для будущих `400`.** `str(exc)` для HTTP-ошибки
   raise_for_status содержит только код и URL, без тела ответа, где
   провайдер обычно и объясняет, что именно не так с запросом. Клиенты
   Groq/OpenRouter (`app/ai/clients.py`) теперь логируют и прокидывают
   фрагмент тела ответа в текст `LLMRequestError` — следующий `400` будет
   виден с причиной, а не только с кодом.

Все три подтверждены тестами на реальных данных: клавиатура рендерится
с кнопкой «Верно», нормализация проверки разделов воспроизводит и чинит
именно репортнутый случай («ИИ функциональность» без дефиса), а
замоканный `400`-ответ теперь попадает в лог и в исключение вместе с
телом.

## Что дальше

- Прогнать сценарии из ТЗ п.66 (Acceptance Criteria) и AI Specification §31
  на реальном боте после того, как заведены все ключи из `.env.example`.
- При желании — заменить дефолтный текст Bot Description (`app/bot/texts.py`)
  и содержимое страницы `/help` (`app/services/telegraph.py`) на финальные
  маркетинговые формулировки.
