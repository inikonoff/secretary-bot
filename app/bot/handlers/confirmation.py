...
async def cb_add_info(
    ...
    async with lock_registry.get(user["id"], application_id):
        ...
        await callback.message.answer(ADD_INFO_PROMPT, reply_markup=keyboards.question_keyboard(application_id))
        await callback.answer()
...
async def cb_confirm_ok(
    ...
    async with lock_registry.get(user["id"], application_id):
        ...
        try:
            ...
            await callback.message.answer(FINALIZATION_FAILED_MESSAGE, reply_markup=keyboards.question_keyboard(application_id))
            return
